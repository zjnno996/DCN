...
TcpAgent::TcpAgent()//zjn cumulative_sent
	: Agent(PT_TCP),
...
{
	/*//zjn 调整包
	if(cumulative_sent>=200*1024)
		size_-=500;
	else if(cumulative_sent>=1024*1024)
		size_-=500;
	size_=max(size_,460);
	//zjn 累计
	cumulative_sent+=nbytes
...
TcpAgent::send_one()
{
	if (t_seqno_ <= highest_ack_ + wnd_ &&   06< curseq_ &&
...
void TcpAgent::recv(Packet *pkt, Handler*)
{
    ...
    if (payload_size == 0) {
        if (tcph->seqno() == last_ack_ + 1) {
            // this is an empty ACK packet, do nothing
           
        } else {
            // this is an empty data packet, send a redundant packet to inform the sender
            output(last_ack_, REASON_TIMEOUT);
            Packet::free(pkt);
            return;
        }
    }*/
