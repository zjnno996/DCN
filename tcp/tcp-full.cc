...
int FullTcpAgent::piasPrio(int bytes_sent)
{
    if(enable_pias_&&pias_prio_num_>=1)
    {
		...
		if(bytes_sent<100*1024) {
			maxseg_=1460;
			return 1;
		}
		else if(bytes_sent<1024*1024) {
			maxseg_=960;
			return 2;
		}
		maxseg_=460;
...
//				iph->prio() = iph->prio() / 40 * 1000 + set_prio(seqno, curseq_) / 1460;
    ...
    if(enable_pias_)
    {
        if(datalen>0)
        {
            ...
            if(pias_debug_)
                printf("Packet size is %d prio is %d when bytes sent is %d\n",ch->size(),iph->prio(),seqno-startseq_);
        }
        else
        {
            iph->prio()=0;
...
send:
	...
	*/
	...
	// highest: greatest sequence number + 1
